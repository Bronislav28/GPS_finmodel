...
<head>
  ...
  <style>
    :root {{ --bg:#f7f8fb; --surface:#fff; --border:#d8deea; --text:#111827; --muted:#4b5563; --accent:#2563eb; }}
    body {{ font-family: Arial, sans-serif; margin: 0; color: var(--text); background: var(--bg); line-height: 1.35; }}
    .page {{ max-width: 1500px; margin: 0 auto; padding: 24px; }}
    h1 {{ margin-bottom: 0.2rem; }}
    h2 {{ margin-top: 2rem; border-bottom: 1px solid var(--border); padding-bottom: 0.3rem; }}
    h3 {{ margin: 0 0 10px; }}
    .table-wrap {{ overflow-x: auto; border: 1px solid var(--border); border-radius: 8px; background: var(--surface); margin: 12px 0 24px; }}
    ...
    .controls {{ display: flex; gap: 16px; margin: 12px 0 18px; flex-wrap: wrap; align-items: center; }}
    label {{ font-weight: bold; font-size: 13px; }}
    select, input[type='range'] {{ margin-left: 8px; padding: 5px; }}
    .card {{ border: 1px solid var(--border); padding: 12px; margin: 12px 0; border-radius: 8px; background: var(--surface); }}
    ...
    .summary-cards {{ display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:10px; margin:12px 0; }}
    .summary-card {{ border:1px solid var(--border); border-radius:8px; background:var(--surface); padding:10px; }}
    .summary-card .k {{ color:var(--muted); font-size:11px; text-transform:uppercase; letter-spacing:.02em; }}
    .summary-card .v {{ font-size:18px; font-weight:700; margin-top:4px; color:var(--accent); }}
  </style>
</head>
<body>
  <div class="page">
  ...
  <div class="card">
    ...
    <div class="controls">
      ...
      <button id="planner-apply" type="button">Apply Scenario</button>
      <button id="planner-reset-base" type="button">Reset to Base</button>
  ...
  <script>
    ...
    const baseMetricsRows = {json.dumps(metrics_rows)};
    ...
    function tableHtml(title, rows) {{
      ...
      return `<h3>${{title}}</h3><div class="table-wrap"><table><thead><tr>${{th}}</tr></thead><tbody>${{body}}</tbody></table></div>`;
    }}

    
    function executiveCardsHtml(summary) {{
      const rows = Object.entries(summary).map(([k,v]) => `<div class="summary-card"><div class="k">${{k.replace(/_/g, ' ')}}</div><div class="v">${{formatValue(v)}}</div></div>`).join('');
      return `<div class="summary-cards">${{rows}}</div>`;
    }}
    ...
    function render(infra, funding) {{
      ...
      document.getElementById('executive-summary').innerHTML = executiveCardsHtml(executive[0]) + tableHtml('Selected scenario snapshot', executive);
    ...
    function formatPayback(monthId) {{
      if (!monthId) return 'Not reached';
      const text = String(monthId);
      return /^\\d{{4}}-\\d{{2}}$/.test(text) ? text : 'Not reached';
    }}

    function calcMetrics(monthlyRows, annualRows, infra, funding) {{
      const discountRateAnnual = Number((((baseAssumptions.financing || {{}}).hurdle_rate || {{}}).annual_discount_rate) || 0);
      const monthlyRate = Math.pow(1 + discountRateAnnual, 1 / 12) - 1;
      let npv = 0;
      let cum = 0;
      let cumDisc = 0;
      ...
      monthlyRows.forEach((r, idx) => {{
        const fcf = Number(r.free_cash_flow_after_financing_costs || 0);
        const invest = Number(r.investing_cash_flow || 0);
        npv += fcf / Math.pow(1 + monthlyRate, idx + 1);
        cum += fcf;
        cumDisc += fcf / Math.pow(1 + monthlyRate, idx + 1);
        if (!simplePayback && cum >= 0) simplePayback = r.month_id;
        if (!discPayback && cumDisc >= 0) discPayback = r.month_id;
        if (invest < 0) requiredInv += Math.abs(invest);
        peakGpu = Math.max(peakGpu, Number(r.required_gpu || 0));
      }});
      const last = monthlyRows.length ? monthlyRows[monthlyRows.length - 1] : {{}};
      return [{{
        infrastructure_scenario: infra,
        funding_scenario: funding,
        npv: Math.round(npv * 100) / 100,
        irr_annualized: null,
        simple_payback_month_key: formatPayback(simplePayback),
        discounted_payback_month_key: formatPayback(discPayback),
        required_investments: Math.round(requiredInv * 100) / 100,
        peak_required_gpu: Math.round(peakGpu * 1e6) / 1e6,
        ending_revolver_balance: Number(last.closing_revolver_balance || 0),
        ending_cash: Number(last.closing_cash_after_funding || 0),
        ending_balance_check: Number(last.balance_check || 0),
      }}];
    }}
    ...
    function onChange() {{
      if (appliedScenario && appliedScenario.active) return;
    ...
    const baseView = {{
      active: false,
      label: 'Base Python-generated scenario',
      annual: [],
      monthly: [],
      metrics: [],
    }};

    function getCurrentBaseRows() {{
      const mode = fundingModeSelect.value;
      if (mode === 'custom_mix') {{
        const eq = Number(equityShareInput.value) / 100;
        const custom = buildCustomView(infraSelect.value, eq);
        return {{ infra: infraSelect.value, funding: 'custom_mix', annual: custom.annual, monthly: custom.monthly }};
      }}
      return {{
        infra: infraSelect.value,
        funding: fundingSelect.value,
        annual: filterRows(reportData.annual_rows, infraSelect.value, fundingSelect.value).sort((a,b)=>a.year-b.year),
        monthly: filterRows(reportData.funded_rows, infraSelect.value, fundingSelect.value).sort((a,b)=>a.month_seq-b.month_seq),
      }};
    }}

    function renderAppliedBanner() {{
      const el = document.getElementById('planner-operating-preview');
      const tag = appliedScenario ? `Applied scenario: ${{appliedScenario.label}}` : 'Applied scenario: none';
      el.insertAdjacentHTML('afterbegin', `<div class="note"><strong>${{tag}}</strong></div>`);
    }}

    function renderFromRows(infra, funding, annual, monthly, label) {{
      const latest = annual.length ? annual[annual.length - 1] : {{}};
      const executive = [{{
        infrastructure_scenario: infra,
        funding_scenario: funding,
    ...
    document.getElementById('planner-apply').addEventListener('click', () => {{
      const base = getCurrentBaseRows();
      const opPreview = recomputeOperatingPreview();
      const opByYear = new Map(opPreview.map((r) => [Number(r.year), r]));
      const monthly = base.monthly.map((r) => {{
        const yr = Number(r.year);
        const y = opByYear.get(yr) || {{}};
        const yearRows = base.monthly.filter((x) => Number(x.year) === yr).length || 1;
        const ratioRev = Number(r.total_revenue || 0) !== 0 ? (Number(y.preview_total_revenue || 0) / yearRows) / Number(r.total_revenue || 1) : 1;
        const ratioSga = Number(r.sga || 0) !== 0 ? (Number(y.preview_sga || 0) / yearRows) / Number(r.sga || 1) : 1;
        const ratioCogs = Number(r.cogs || 0) !== 0 ? (Number(y.preview_cogs || 0) / yearRows) / Number(r.cogs || 1) : 1;
        const out = {{...r}};
        out.total_revenue = Math.round(Number(r.total_revenue || 0) * ratioRev * 100) / 100;
        out.cogs = Math.round(Number(r.cogs || 0) * ratioCogs * 100) / 100;
        out.sga = Math.round(Number(r.sga || 0) * ratioSga * 100) / 100;
        out.ebit = Math.round((out.total_revenue - out.cogs - out.sga - Number(r.depreciation_and_amortization || 0)) * 100) / 100;
        out.ebt_after_interest = Math.round((out.ebit - Number(r.interest_expense || 0)) * 100) / 100;
        out.profit_tax_after_interest = Math.round(Math.max(0, out.ebt_after_interest) * 0.25 * 100) / 100;
        out.net_income_after_interest = Math.round((out.ebt_after_interest - out.profit_tax_after_interest) * 100) / 100;
        out.operating_cash_flow_after_interest = Math.round((out.net_income_after_interest + Number(r.depreciation_and_amortization || 0)) * 100) / 100;
        out.free_cash_flow_after_financing_costs = Math.round((out.operating_cash_flow_after_interest + Number(r.investing_cash_flow || 0) + Number(r.equity_injection || 0) + Number(r.revolver_drawdown || 0) - Number(r.revolver_repayment || 0) - Number(r.interest_expense || 0)) * 100) / 100;
        out.closing_cash_after_funding = Math.round((Number(r.opening_cash_before_funding || 0) + out.free_cash_flow_after_financing_costs) * 100) / 100;
        out.cash = out.closing_cash_after_funding;
        out.balance_check = Math.round((Number(r.total_assets || 0) - Number(r.total_liabilities || 0) - Number(r.total_equity || 0)) * 100) / 100;
        return out;
      }});
      const annualMap = new Map();
      monthly.forEach((r) => {{
        const k = Number(r.year);
        if (!annualMap.has(k)) annualMap.set(k, {{infrastructure_scenario: base.infra, funding_scenario: base.funding, year: k, months_in_year: 0, total_revenue: 0, net_income_after_interest: 0, closing_cash_after_funding: 0, closing_revolver_balance: 0}});
        const a = annualMap.get(k);
        a.months_in_year += 1;
        a.total_revenue += Number(r.total_revenue || 0);
        a.net_income_after_interest += Number(r.net_income_after_interest || 0);
        a.closing_cash_after_funding = Number(r.closing_cash_after_funding || 0);
        a.closing_revolver_balance = Number(r.closing_revolver_balance || 0);
      }});
      const annual = Array.from(annualMap.values()).sort((a,b)=>a.year-b.year).map((r)=>({{...r,total_revenue:Math.round(r.total_revenue*100)/100,net_income_after_interest:Math.round(r.net_income_after_interest*100)/100}}));
      const metrics = calcMetrics(monthly, annual, base.infra, base.funding);
      appliedScenario = {{active: true, label: `override @ ${{new Date().toISOString()}}`, annual, monthly, metrics}};
      renderFromRows(base.infra, base.funding, annual, monthly, appliedScenario.label);
      document.getElementById('planner-operating-preview').innerHTML = tableHtml('Applied scenario investment metrics', metrics) + document.getElementById('planner-operating-preview').innerHTML;
      renderAppliedBanner();
    }});
    document.getElementById('planner-reset-base').addEventListener('click', () => {{
