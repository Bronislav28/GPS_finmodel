...
(function initScenarioLab(){
  try {
    ...
    const applyScenarioLabState=(st)=>{ if(!st) return; Object.entries(st.scalars||{}).forEach(([k,v])=>{ const e=document.getElementById(k); if(e) e.value=String(v); });
      ...
      Object.entries(st.key_assumptions||{}).forEach(([k,ym])=>{ Object.entries(ym||{}).forEach(([y,v])=>{ const e=document.querySelector(".sl-key-assumption-input[data-assumption-key='"+k+"'][data-year='"+y+"']"); if(e) e.value=Number(v)||0; }); });
