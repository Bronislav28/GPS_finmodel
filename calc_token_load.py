...
OPERATING_MONTHLY_WHITELIST = {
    ...
    "discount_rate_annual", "discount_rate_monthly",
    ...
    "investing_cash_flow", "operating_cash_flow", "closing_cash_before_funding", "free_cash_flow",
    "ebt", "profit_tax", "net_income", "discount_factor", "discounted_fcf", "cumulative_discounted_fcf",
    "roic", "roe", "roa", "debt_to_equity", "net_debt", "net_debt_to_ebitda", "interest_coverage",
}
    ...
    for mo in months:
        ...
        monthly = {
            **mo,
            ...
            "minimum_cash_balance": min_cash, "revolver_interest_rate": annual_interest_rate, "profit_tax_rate": (as_float(r.get("profit_tax_rate")) or 0.0), "discount_rate": discount_annual,
...
"""
    ...
    debt_cfg = ((assumptions.get("funding", {}) or {}).get("debt", {}) or {})
    config_revolver_rate = float(as_float(((debt_cfg.get("revolver_interest_rate", {}) or {}).get("value")) or 0.0) or 0.0)
    config_profit_tax_rate = float(as_float((((assumptions.get("pnl", {}) or {}).get("tax", {}) or {}).get("profit_tax_rate", {}).get("value")) or 0.0) or 0.0)
    config_discount_rate = float(as_float((((assumptions.get("investment_metrics", {}) or {}).get("discount_rate", {}) or {}).get("value", {}).get(years[0])) or 0.0) or 0.0)
...
(function(){{
  ...
  window.REPORT_SCENARIO_CONFIG = Object.assign({{}}, window.REPORT_SCENARIO_CONFIG||{{}}, {{
    ...
    revolver_interest_rate: {config_revolver_rate},
    profit_tax_rate: {config_profit_tax_rate},
    discount_rate: {config_discount_rate},
  ...
  const renderFinancialFlow=(year)=>{{ if(!ffPlot) return; const d=FIN_FLOW[String(year)]||{{}}; const rev=ffVal(d,'total_revenue');
    ...
    const firstNum=(...vals)=>{{ for(const v of vals){{ const n=Number(v); if(Number.isFinite(n)) return n; }} return null; }};
    ...
    const buildFundedMonthlyRows=(operatingRows,fundingScenario,equitySharePct,operatingKey='unknown')=>{{ const rows=JSON.parse(JSON.stringify(operatingRows||[])).sort((a,b)=>Number(a.month_index||0)-Number(b.month_index||0)); const shares=getFundingShares(fundingScenario,equitySharePct); let priorCumDisc=0, priorCumFcf=0;
      ...
      rows.forEach((r,idx)=>{{ const prev=idx>0?rows[idx-1]:null; const opening_cash=prev?Number(prev.closing_cash_after_funding||0):0; const opening_revolver_balance=prev?Number(prev.revolver_balance||0):0; const opening_paid_in_capital=prev?Number(prev.paid_in_capital||0):0; const opening_retained_earnings=prev?Number(prev.retained_earnings||0):0;
        ...
        const monthlyDiscountRate=getMonthlyDiscountRate(r); const discount_factor=1/Math.pow(1+(monthlyDiscountRate||0),idx); const discounted_fcf=free_cash_flow*discount_factor; priorCumDisc += discounted_fcf; priorCumFcf += free_cash_flow;
    ...
    const getMonthKey=(row)=>{{ if(row?.month_key) return String(row.month_key); const y=Number(row?.year), m=Number(row?.month); if(Number.isFinite(y)&&Number.isFinite(m)) return `${{y}}-${{String(m).padStart(2,'0')}}`; return 'N/A'; }};
    const buildAnnualInvestmentSummaryFromMonthly=(monthlyRows,annualRows)=>{{ const m=(monthlyRows||[]).sort((a,b)=>Number(a.month_index||0)-Number(b.month_index||0)); const npv=m.length?Number(m[m.length-1].cumulative_discounted_fcf||0):0; let cum=0,sp='Not reached',dp='Not reached'; m.forEach(r=>{{ cum += Number(r.free_cash_flow||0); if(sp==='Not reached'&&cum>=0) sp=getMonthKey(r); if(dp==='Not reached'&&Number(r.cumulative_discounted_fcf||0)>=0) dp=getMonthKey(r); }});
    ...
    const renderMonthlyDetail=()=>{{ const y=String((document.getElementById('monthly_detail_year')||{{}}).value||YEARS[0]); const sec=String((document.getElementById('monthly_detail_section')||{{}}).value||'Demand & Tokens'); const host=document.getElementById('monthly_detail_table'); const st=document.getElementById('monthly_detail_status'); if(!host) return; if(!(currentMonthlyRows||[]).length){{ host.innerHTML=\"<div class='note warn'>No monthly rows available for the selected scenario.</div>\"; if(st) st.textContent='No monthly rows available for the selected scenario.'; return; }} const rows=(currentMonthlyRows||[]).filter(r=>String(r.year)===y).sort((a,b)=>Number(a.month)-Number(b.month)); if(!rows.length){{ host.innerHTML=`<div class='note warn'>No monthly rows for selected year: ${{y}}.</div>`; if(st) st.textContent=`No monthly rows for selected year: ${{y}}.`; return; }} const metrics=MONTH_METRICS[sec]||[]; if(!metrics.length){{ host.innerHTML=`<div class='note warn'>No metrics configured for section: ${{sec}}.</div>`; if(st) st.textContent=`No metrics configured for section: ${{sec}}.`; return; }} const mnames=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; const mons=rows.map(r=>mnames[Math.max(0,Number(r.month)-1)]+' '+String(r.year)); const head='<tr><th class=\"sticky\">Metric</th>'+mons.map(m=>'<th>'+m+'</th>').join('')+'<th>FY Total / YE</th></tr>'; const body=metrics.map(m=>{{ const vals=rows.map(r=>Number(r[m])||0); const t=monthlyMetricAggregationType(m); const agg=t==='sum'?vals.reduce((a,b)=>a+b,0):t==='max'?Math.max(...vals,0):t==='avg'?(vals.reduce((a,b)=>a+b,0)/Math.max(vals.length,1)):(vals.length?vals[vals.length-1]:0); return '<tr><td class=\"sticky\">'+m+'</td>'+vals.map(v=>'<td class=\"num\">'+formatMonthlyValue(m,v)+'</td>').join('')+'<td class=\"num\">'+formatMonthlyValue(m,agg)+'</td></tr>'; }}).join(''); host.innerHTML='<table class=\"sensitivity\"><thead>'+head+'</thead><tbody>'+body+'</tbody></table>'; }};
  ...
  try {{
    ...
    if(reset) reset.addEventListener('click',()=>{{ if(infra) infra.value=baseInfra; if(fund) fund.value=baseFunding; if(csy) csy.value=String(baseCsy); if(csm) csm.value=String(baseCsm); if(eq) eq.value=String(Math.round(baseEq)); sync(); updateInvestmentScenarioControlState(); applyReportScenario(); try{{updateMonthlyDetailFromControls();}}catch(err){{console.error(err);}} if(st) st.textContent='Reset to YAML base investment scenario. Monthly Detail updated.'; }});
